...
class DB(object):
	...
	def execute(self, operation, data=None):
		with self.driver.session() as session:
			...
			elif operation == OPS.POST_APPROVED_CONNECTION:
				return session.execute_write(self._post_approved_connection, data)
			elif operation == OPS.GET_APPROVED_CONNECTIONS:
				return session.execute_write(self._get_approved_connections, data)
			else:
